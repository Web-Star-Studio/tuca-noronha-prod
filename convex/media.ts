 ...
 * This is useful when the URL has expired or is no longer valid
 */
export const refreshStorageUrl = mutation({
  args: {
    id: v.id("media"),
  },
  returns: v.string(),
  handler: async (ctx, args) => {
    // Obter o registro de mídia
    const media = await ctx.db.get(args.id);
    ...
    // Gerar uma nova URL para o arquivo no storage
    const newUrl = await ctx.storage.getUrl(media.storageId);
    
    if (!newUrl) {
      throw new Error(`Failed to generate URL for storage ID ${media.storageId}`);
    }
    ...
    console.log(`URL refreshed for media ${args.id}: ${newUrl}`);
