import { Accommodation } from "@/lib/services/accommodationService";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardFooter } from "@/components/ui/card";
import { Menu, Bed, Bath, Users } from "lucide-react";
import Image from "next/image";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuSeparator,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { motion } from "framer-motion";
import { imageEffects } from "@/lib/ui-config";

interface AccommodationCardProps {
  accommodation: Accommodation;
  onEdit: (accommodation: Accommodation) => void;
  onDelete: (id: string) => void;
  onToggleFeatured: (id: string, featured: boolean) => void;
  onToggleActive: (id: string, active: boolean) => void;
}

export function AccommodationCard({
  accommodation,
  onEdit,
  onDelete,
  onToggleFeatured,
  onToggleActive,
}: AccommodationCardProps) {
  // Get accommodation ID from _id or id
  const accommodationId = accommodation._id?.toString() || accommodation.id || "";

  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -20 }}
      transition={{ duration: 0.3 }}
      layout
    >
      <Card variant="interactive" className="h-full relative group">
        {/* Card header with image */}
        <div className="relative h-48 w-full bg-gray-100 overflow-hidden">
          <Image
            src={accommodation.mainImage || "https://via.placeholder.com/300x200?text=No+Image"}
            alt={accommodation.name}
            fill
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            className={`object-cover ${imageEffects.hover.scale}`}
          />

          {/* Badges overlay */}
          <div className="absolute top-2 left-2 flex flex-wrap gap-1">
            <Badge 
              variant={accommodation.isActive ? "success" : "outline"}
            >
              {accommodation.isActive ? "Ativa" : "Inativa"}
            </Badge>
            
            {accommodation.isFeatured && (
              <Badge variant="warning">
                Destacada
              </Badge>
            )}
          </div>

          {/* Price per night badge */}
          <div className="absolute top-2 right-2">
            <Badge variant="outline" className="bg-white/80 backdrop-blur-sm text-gray-900 font-bold">
              R$ {accommodation.pricing?.pricePerNight}/noite
            </Badge>
          </div>

          {/* Action menu */}
          <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="sm" variant="secondary" className="h-8 w-8 p-0 rounded-full bg-white/80 backdrop-blur-sm hover:bg-white">
                  <Menu className="h-4 w-4" />
                  <span className="sr-only">Ações</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-40">
                <DropdownMenuItem onClick={() => onEdit(accommodation)}>
                  Editar
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onToggleFeatured(accommodationId, !accommodation.isFeatured)}>
                  {accommodation.isFeatured ? "Remover destaque" : "Destacar"}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onToggleActive(accommodationId, !accommodation.isActive)}>
                  {accommodation.isActive ? "Desativar" : "Ativar"}
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem 
                  className="text-red-600 focus:text-red-700" 
                  onClick={() => onDelete(accommodationId)}
                >
                  Excluir
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
          
          <div className={imageEffects.overlay.dark}></div>
        </div>

        {/* Card content */}
        <CardContent>
          <div className="space-y-2">
            <h3 className="font-semibold text-lg">{accommodation.name}</h3>
            
            <div className="flex items-center text-sm text-gray-500">
              <span>{accommodation.address?.neighborhood}, {accommodation.address?.city} - {accommodation.address?.state}</span>
            </div>

            <p className="text-sm text-gray-700 line-clamp-2">{accommodation.description}</p>

            {/* Room info */}
            <div className="flex items-center gap-4 text-sm text-gray-600 pt-1">
              <div className="flex items-center gap-1">
                <Bed className="h-4 w-4" />
                <span>{accommodation.rooms?.bedrooms} quartos</span>
              </div>
              <div className="flex items-center gap-1">
                <Bath className="h-4 w-4" />
                <span>{accommodation.rooms?.bathrooms} banheiros</span>
              </div>
              <div className="flex items-center gap-1">
                <Users className="h-4 w-4" />
                <span>{accommodation.maximumGuests} hóspedes</span>
              </div>
            </div>

            <div className="flex flex-wrap gap-1 pt-2">
              <Badge variant="outline" className="bg-purple-50 text-purple-700">
                {accommodation.type}
              </Badge>
              {accommodation.amenities?.slice(0, 2).map((amenity, index) => (
                <Badge key={index} variant="outline" className="bg-blue-50 text-blue-700">
                  {amenity}
                </Badge>
              ))}
              {accommodation.amenities && accommodation.amenities.length > 2 && (
                <Tooltip>
                  <TooltipTrigger>
                    <Badge variant="outline" className="bg-gray-50 text-gray-600 cursor-help">
                      +{accommodation.amenities.length - 2}
                    </Badge>
                  </TooltipTrigger>
                  <TooltipContent>
                    {accommodation.amenities.slice(2).join(", ")}
                  </TooltipContent>
                </Tooltip>
              )}
            </div>
          </div>
        </CardContent>

        <CardFooter separator={true}>
          <div className="flex items-center space-x-1">
            <span className="text-yellow-500">★</span>
            <span className="font-medium">{accommodation.rating?.overall.toFixed(1)}</span>
            <span className="text-gray-500 text-sm">({accommodation.rating?.totalReviews})</span>
          </div>

          <div className="flex space-x-2">
            <Button 
              variant="outline" 
              size="sm" 
              className="h-8 text-xs text-blue-700 hover:bg-blue-50"
              onClick={() => onEdit(accommodation)}
            >
              Gerenciar
            </Button>
          </div>
        </CardFooter>
      </Card>
    </motion.div>
  );
} 